...
object EsAlgebra {
  def impl[F[_]: Async: Console](esConfig: EsConfig, restHighLevelClient: RestHighLevelClient)
  : EsAlgebra[F] =
    new EsAlgebra[F] {
      override def createIndex: F[Unit] = for {
        getIndex <- getIndexRequest
        indexExit <- Async[F].delay(restHighLevelClient.indices().exists(getIndex, RequestOptions.DEFAULT))
      ...
      override def store(event: Event): F[IndexResponse] = Async[F].delay{
