...
object KafkaConsumerAlgebra {
  def impl[F[_]: Async: Console] (kafkaConfig: KafkaConfig)
    = new KafkaConsumerAlgebra[F] {
    def consume: Stream[F, CommittableConsumerRecord[F, Unit, Event]] = {

      val valueDeserializer: Deserializer[F, Event]
      = Deserializer.lift[F, Event] (byteArray => Async[F].fromEither(decodeByteArray[Event](byteArray)))
      ...
      KafkaConsumer
        ...
        => Console[F].println(
            s"Record has been processed -> ${consumedRecord.record.value}"
